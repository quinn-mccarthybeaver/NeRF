...
class Nerf(nn.Module):
    ...
    def forward(self, xyz, d):
        ...
        sigma = h[:, -1] #extracting sigma
        h = h[:, :-1]  # [batch_size, hidden_dim]  # taking the last 256 hidden
        c = self.rgb_head(torch.cat((h, d_emb), dim=1))

        return c, torch.relu(sigma)  # relu is used to get a density that is always greater than 0
